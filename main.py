...
def get_start_kb() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        ...
        keyboard=[[KeyboardButton(text=_("Yangi CV/Rezyume yaratish"))]]
    )
...
def check_year(text: str) -> bool:
    return bool(re.match(r"^\d{4}\s-\s\d{4}$", text))
...
@dp.message(F.text.in_(CV_START_TEXTS))
async def cv_start(message: Message, state: FSMContext):
    await message.answer(_("Ism va Familiyangizni kiriting:"), reply_markup=ReplyKeyboardRemove())
...
async def get_full_name(message: Message, state: FSMContext):
    ...
    await message.answer(_("Manzilingizni kiriting (shahar, tuman):"))
...
@dp.message(UserStates.user_phone)
...
async def get_email(message: Message, state: FSMContext):
    ...
    await state.set_state(UserStates.user_profession)


@dp.message(UserStates.user_profession)
...
async def edu_name(message: Message, state: FSMContext):
    ...
    await message.answer(_("Qaysi yillarda o'qigansiz? (masalan: 2020 - 2024)"))
    await state.set_state(UserStates.edu_years)


@dp.message(UserStates.edu_years)
async def edu_years(message: Message, state: FSMContext):
...
async def edu_field(message: Message, state: FSMContext):
    ...
    education.append(data["current_edu"])
    await state.update_data(education=education, current_edu=None)
...
async def edu_again(message: Message, state: FSMContext):
    if message.text.lower() in YES_ANSWERS:
        await message.answer(_("Qayerda o'qigansiz?"), reply_markup=ReplyKeyboardRemove())
        await state.set_state(UserStates.edu_name)
    else:
        await message.answer(_("Nimalarni bilasiz (vergul bilan):"), reply_markup=ReplyKeyboardRemove())
...
async def get_skills(message: Message, state: FSMContext):
    ...
    await message.answer(_("Portfolio/GitHub uchun link kiriting (ixtiyoriy):"), reply_markup=skip_kb)
...
async def skip_links(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(_("Qaysi tillarni bilasiz (vergul bilan):"))
...
async def get_links(message: Message, state: FSMContext):
    ...
    await message.answer(_("Qaysi tillarni bilasiz (vergul bilan):"))
    await state.set_state(UserStates.user_language)
...
async def get_language(message: Message, state: FSMContext):
    ...
    await message.answer(_("O'zingiz haqingizda ma'lumot kiriting:"))
...
async def work_name(message: Message, state: FSMContext):
    ...
    await message.answer(_("Qaysi yillarda ishladingiz? (masalan: 2020 - 2024)"))
...
async def work_again(message: Message, state: FSMContext):
    if message.text.lower() in YES_ANSWERS:
